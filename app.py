                    ...
                    if service.image and service.image != 'default-service.jpg':
                        try:
                            old_image_path = os.path.join(
                                app.config['UPLOAD_FOLDER'], service.image.split('/')[-1])
...
def get_yogaservices():
    try:
        ...
        if instructor:
            query = query.filter(
                yogaService.instructor.ilike(f'%{instructor}%'))
...
def admin_reviews():
    ...
    if request.method == 'POST':
        ...
        reviewer_image = save_file(
            request.files['reviewer_image']) if 'reviewer_image' in request.files else None
...
with app.app_context():
    if not User.query.first():
        ...
        db.session.commit()
        print(
            "Default admin user created - username: wellnation360, password: wellnation@360")
...
def gallery():
    ...
    distinct_categories = db.session.query(
        GalleryItem.category).distinct().all()
    ...
    for category_tuple in distinct_categories:
        ...
        items_from_db = GalleryItem.query.filter_by(
            category=category_name_str).all()
...
def index():
    ...
    for service in services_from_db:
        ...
        if service.image_filename:
            image_url = url_for('static', filename=os.path.join(
                'uploads', service.image_filename))

        # Get plans and find the one with the minimum months for display
        service_plans = service.plans.order_by(
            PricingPlan.months.asc()).all()  # Use .all() with lazy='dynamic'
        plans_list_for_json = []
        ...
        if service_plans:
            ...
            for p in service_plans:
                plans_list_for_json.append(
                    {"months": p.months, "price": p.price})
...
# JS uses POST, could be PUT
@app.route('/api/service/update/<int:service_id>', methods=['POST'])
def api_update_service(service_id):
    ...
    try:
        ...
        service.instructor_name = request.form.get(
...
# JS uses POST, could be DELETE
@app.route('/api/service/delete/<int:service_id>', methods=['POST'])
def api_delete_service(service_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error deleting service {service_id}: {str(e)}")
...
def get_reviews():
    ...
    for review_db in reviews_from_db:
        ...
        if review_db.reviewer_image:
            image_path = os.path.join(
                'uploads', review_db.reviewer_image).replace('\\', '/')
            reviewer_image_url = url_for(
                'static', filename=image_path, _external=True)

        media_url = None
        if review_db.media_path:
            media_path = os.path.join(
                'uploads', review_db.media_path).replace('\\', '/')
...
def book_service():
    try:
        ...
        if not SENDER_EMAIL or not SENDER_PASSWORD or not RECIPIENT_EMAIL:
            logger.error(
                "Email credentials (SENDER_EMAIL, SENDER_PASSWORD, RECIPIENT_EMAIL) are not set in .env")
            return jsonify({
        ...
        logger.error(
            "SMTP Authentication Error - Check SENDER_EMAIL and SENDER_PASSWORD (use App Password for Gmail)")
        return jsonify({
